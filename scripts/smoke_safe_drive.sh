#!/usr/bin/env bash
# Smoke test end-to-end: Gazebo custom_casa + maze_nav safe_drive.

set -eo pipefail

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
ROOT_DIR="$(cd "${SCRIPT_DIR}/.." && pwd)"
SIM_LOG="/tmp/maze_nav_safe_drive_sim.log"
NAV_LOG="/tmp/maze_nav_safe_drive_nav.log"
...
cleanup() {
  ...
  "${SCRIPT_DIR}/kill_all.sh" >/tmp/maze_nav_safe_drive_cleanup.log 2>&1 || true
}
trap cleanup EXIT

echo ">>> Limpiando procesos previos"
"${SCRIPT_DIR}/kill_all.sh" >/tmp/maze_nav_safe_drive_kill.log 2>&1 || true
...
echo ">>> Lanzando maze_nav safe_drive"
...
set +e
python3 "${SCRIPT_DIR}/smoke_safe_drive_client.py" "$@"
STATUS=$?
set -e
